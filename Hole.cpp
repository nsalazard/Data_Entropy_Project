...
void evolve(vector &data, vector &prob, int Np, int N, int nsteps, double Xmin,
            double Ymin, double DELTA, int Npart, int u, int hole);
...
void evolve(vector &data, vector &prob, int Np, int N, int nsteps, double Xmin,
  ...
  if (u == 1) { // Print the entropy                                                                                 
    ...
    for (int istep = 1; istep <= nsteps; istep += 1) {                                                               
      ...
      // print_screen(data, N);                                                                                      
      if (istep % 100 == 0) {                                                                                        
        std::cout << istep << "\t" << grid(data, prob, Np, N, Npart)                                                 
                  << std::endl;                                                                                      
        initial_conditions(prob, Np);                                                                                
      } 
      }                                                                                                                
